...
public class Calculator {
		...
		while (true) {
			...
			if (!input.equals("e")) {	// If input isn't 'e', continue with program.
				...
				if (infix == "e") {		// If a broken arithmetic expression was given. Proceed to next iteration of the program.
					System.out.println();
					System.out.println("This calculator wont allow that");
	...
	public static String computePostfix(String input) {
		...
		for (int i = 0; i < input.length(); i++) {// For-loop over the length of input.
			if (Character.isDigit(input.charAt(i))) { // When digit. Update B with 'i' of input.
				...
				break;
				case ('-'):
					if (!B.isEmpty()) { 	// If B isn't empty. Empty A+B
						A = (Integer.parseInt(A) - Integer.parseInt(B)) + "";
						B = "";
				...
				case ('/'):
					if (!B.isEmpty()) {
						if(A.charAt(0)=='0' && B.charAt(0)=='0') {	// In case of 0/0
							return "0";
						}else if (B.charAt(0)=='0') { // In case of A/0 
							return "e";

						}else {	// If B isn't empty and the first character in B isn't '0'.  A/B
							A = (Integer.parseInt(A) / Integer.parseInt(B)) + "";
							B = "";
						} 
					}
	...
	public static String checkInput(String input) {
		...
		for (int i = 1; i < input.length(); i++) { 
			...
			} else if (Character.isLetter(input.charAt(i))) { // Not in version 1.0 Added after submission of assignment1
				return "e";
